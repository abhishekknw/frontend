...
.controller('NewSocietyCtrl',
    ...
    function ($scope, $rootScope, $window, $location, newsocietyDetailService, constants) {
     ...
     newsocietyDetailService.getSociety($rootScope.societyId,$scope.supplier_type_code)
     ...
     newsocietyDetailService.get_inventory_summary($rootScope.societyId, $scope.supplier_type_code)
     .success(function (response){
    ...
    if($rootScope.campaignId){
        newsocietyDetailService.getShortlistedSocietyCount($rootScope.campaignId)
    ...
    $scope.society_ids = {}
    newsocietyDetailService.getSocietyIds()
    .success(function(response,status){
    ...
    $scope.getSecondIndex = function(Images,index)
    {
     ...
     if($rootScope.campaignId){
       ...
       newsocietyDetailService.addShortlistedSociety($rootScope.campaignId, id)
        .success(function (response){
            // $scope.model = response;
              // $location.path("manageCampaign/shortlisted/" + $rootScope.campaignId + "/societies");
              $scope.disable = true;
