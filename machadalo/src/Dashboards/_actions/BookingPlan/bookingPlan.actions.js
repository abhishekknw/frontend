...
const BookinPlanActions = () => {
  ...
  const getCommetByShortlistedId = (data, type) => {
    let params = `shortlisted_spaces_id=${data?.id}&related_to=${
      type === 'externalComments' ? 'EXTERNAL' : 'INTERNAL'
    }`;
  ...
  const getPermissionBoxImages = (data) => {
    return fetchWrapper
      .get(
        `${Apis.Get_Permission_Box_Images}?campaign_id=${data?.campaign_id}&supplier_id=${data?.supplier_id}`
      )
      .then((res) => {
        if (res.status) {
          return res.data;
  ...
  const postPermissionBoxImages = (file) => {
    return fetchWrapper
      .post(
        `v0/ui/website/hashtag-images/${CampaignProposalId}/${Apis.Post_permission_Box_Images}`,
        file,
        true
      )
      .then((res) => {
  ...
  const getReceiptImages = (data) => {
    return fetchWrapper
      .get(
        `${Apis.Get_Receipt_Images}?campaign_id=${data?.campaign_id}&supplier_id=${data?.supplier_id}`
      )
      .then((res) => {
        if (res.status) {
          return res.data;
  ...
  const postReceiptImages = (file) => {
    return fetchWrapper
      .post(
        `v0/ui/website/hashtag-images/${CampaignProposalId}/${Apis.Post_Receipt_Images}`,
        file,
        true
      )
      .then((res) => {
  ...
  const getInvetoryList = (id) => {
    return fetchWrapper
      .get(`${Apis.Get_Inventory_List}?shortlisted_spaces_id=${id}`)
      .then((res) => {
        if (res?.status) {
