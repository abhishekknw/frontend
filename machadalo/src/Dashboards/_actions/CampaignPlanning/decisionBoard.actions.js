...
const DecisionBoardActions = () => {
  ...
  const converCampaignToProposal = (data) => {
    return fetchWrapper
      .post(`v0/ui/website/${data.proposal.proposal_id}/convert-to-proposal/`, data.proposal)
      .then((res) => {
        if (res.status) {
  ...
  const convertProposalToCampaign = (data) => {
    return fetchWrapper
      .post(`v0/ui/website/${data.proposal.proposal_id}/convert-to-campaign/`, data.proposal)
      .then((res) => {
        if (res.status) {
          alertActions.success(res.data);
          let newList = invoiceProposals.list.map((item, key) => {
  ...
  const convertCampaignOnHold = (data) => {
    ...
    return fetchWrapper
      .put(`v0/ui/website/proposal/${data.proposal.proposal_id}/`, data.proposal)
      .then((res) => {
        if (res.status) {
  ...
  function getOrganisationList() {
    return fetchWrapper.get(`${Apis.Get_Organisations}`).then((res) => {
      if (res.status) {
        let newList = res.data.map(({ name: label, organisation_id: value, ...rest }) => ({
          label,
          value,
          ...rest,
        }));
  ...
  const getUserMinimalList = (id) => {
    return fetchWrapper.get(`${Apis.Get_User_Minimal_List}?org_id=${id}`).then((res) => {
      if (res.status) {
        let newList = res.data.map(({ first_name: label, id: value, ...rest }) => ({
          label,
          value,
          ...rest,
        }));
