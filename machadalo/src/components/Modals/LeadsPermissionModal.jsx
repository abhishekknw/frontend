...
export default class PermissionModal extends React.Component {
  ...
  componentWillMount() {
    if (this.props.createPermission) {
      ...
      this.props.getLeadUserPermission(this.props.modalUserId);
    }
  }
  ...
  componentDidUpdate() {
    if (
      (!this.state.data.data.length &&
        this.props.leads.userPermission.length) ||
      (this.state.data.data.length &&
        this.props.leads.userPermission.length &&
        this.state.data.data[0].entityId !==
          this.props.leads.userPermission[0].entityId)
    ) {
      let dataInfo = this.state.data;
      dataInfo.data = this.props.leads.userPermission;
      this.setState({
        data: dataInfo,
