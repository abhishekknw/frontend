...
//       sortingKey: '',
//       sortReverse: false,
//       pagination: { page: 1, startIndex: 0, endIndex: 10, total_count: 0 },
//     };
...
//       .get(
//         `${config.API_URL}/v0/ui/ops/get-supplier-summary-agencywise/?next_page=${this.state.pagination.page}`
//       )
//       .set('Authorization', `JWT ${token}`)
//       .then((resp) => {
//         const { status, data } = resp.body;
//         if (status) {
//           const entityData = Object.keys(data?.list).map((key, index) => ({
//             ...data.list[key],
//             type: key,
//             key: index,
//           }));
...
//             entityData,
//             isDataFetched: true,
//             pagination: { ...this.state.pagination, total_count: data?.total_count },
//           });
//         }
//       })
//       .catch((ex) => {
...
//               pageSize={10}
//               totalItems={this.state.pagination.total_count}
...
export default function EntityCount() {
  ...
  const getEntityData = async (filters) => {
    await fetchWrapper
      ...
      .catch((ex) => {
        console.log('Failed to get data');
        // ErrorWrapper = ({ children, error, errorTop, errorBottom})
        setIsDataFetched(true);
        <ErrorWrapper children={'Failed to get data'} error={ex} errorTop={true} />;
      });
  };
